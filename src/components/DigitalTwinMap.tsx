...
export default function DigitalTwinMap() {
  ...
  return (
    <div className="relative w-full min-h-[450px] lg:min-h-[550px] bg-black flex items-center justify-center p-4 overflow-hidden rounded-3xl border border-slate-800">
      {/* Background Ambience */}
      ...
      {/* Simulation Engine Control Panel (Bottom Right) */}
      <div className="absolute bottom-4 right-4 z-50 w-72 pointer-events-auto">
        <SimulationEngine onTrigger={setSimulation} />
      </div>

      {/* Layer Controls (Top Left) */}
      <div className="absolute top-4 left-4 z-50 flex flex-wrap gap-2 pointer-events-auto">
        {(['density', 'medical', 'security', 'accessibility', 'temperature'] as const).map(layer => (
          <button 
            ...
            className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${
              activeLayer === layer 
                ? 'bg-[#E20074] text-white shadow-[0_0_10px_rgba(226,0,116,0.5)]' 
      ...
      <div className="relative w-full max-w-3xl aspect-[4/3] rounded-[80px] sm:rounded-[120px] border-4 border-slate-800/80 bg-[#0A0015]/60 shadow-[0_0_50px_rgba(43,124,228,0.1)] flex items-center justify-center backdrop-blur-md">
        
        {/* Seat Intelligence */}
        <SeatIntelligence simulation={simulation} />

        {/* Pitch Area */}
        <div className="w-1/2 h-2/3 border-2 border-white/10 bg-[#1AA65D]/5 rounded-xl flex items-center justify-center relative shadow-[inset_0_0_50px_rgba(26,166,93,0.1)]">
          <PitchHeatmap mode={activeLayer} simulation={simulation} />
          
          <div className="w-20 h-20 border-2 border-white/20 rounded-full z-10"></div>
